...
fn test_update_student_class() {
    ...
    setup_result
        .client
        .register_student(&setup_result.student_wallet, &name, &Class::Grade);

    let student_id = 1u64;

    // Confirm initial class is Grade
    let student = setup_result.client.get_student(&student_id);
    assert!(matches!(student.class_name, Class::Grade));
    ...
    let updated = setup_result.client.get_student(&student_id);
    assert!(matches!(updated.class_name, Class::HighSchool));
}
...
fn test_get_student_payment_history() {
    ...
    setup_result
        .token_client
        .mint(&setup_result.student_wallet, &(amount * 2));
    ...
    assert_eq!(history.get(0).unwrap().student_id, student_id);
}
...
fn test_remove_student() {
    ...
    // Attempting to get the removed student should panic (unwrap on None)
